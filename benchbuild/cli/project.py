...
class BBProjectView(cli.Application):
    ...
    @cli.switch(
        ["-G", "--group"], str, list=True, help="Include projects of this group."
    )
...
class BBProjectDetails(cli.Application):
    ...
    @cli.switch(["-l", "--limit"], int, help="Limit the number of versions to display")
...
def print_projects(projects: ProjectIndex) -> None:
    ...
    project_row_format = (
        "{name:<{width}} | {domain:^15} | {num_sources:^15} | {description}"
    )
