...
class BenchBuildRun(cli.Application):
    ...
    test_full = cli.Flag(
        ["-F", "--full"], help="Test all experiments for the project", default=False
    )
    ...
    @cli.switch(
        ["-D", "--description"], str, help="A description for this experiment run"
    )
    ...
    def main(self, *projects: str) -> int:
        ...
        if self.test_full:
            ...
            exps = dict(
                filter(lambda pair: pair[0] in set(experiment_names), all_exps.items())
            )

        unknown_exps = list(
            filter(lambda name: name not in all_exps.keys(), set(experiment_names))
        )
        if unknown_exps:
            print("Could not find ", str(unknown_exps), " in the experiment registry.")
