...
def _spawn_add_layer(
    ...
    with local.cwd(container.context):
        sources = [os.path.join(container.context, source) for source in layer.sources]
        buildah_add = bb_buildah("add", "--add-history")
        buildah_add = buildah_add[container.container_id][sources][layer.destination]
...
def _set_working_directory(
    ...
    return run(
        bb_buildah("config")["--workingdir", layer.directory, container.container_id]
    )


LayerHandlerT = tp.Callable[
    [model.Container, model.Layer], Result[str, ProcessExecutionError]
]
...
def spawn_layer(
    ...
    handler: LayerHandlerT = tp.cast(LayerHandlerT, _LAYER_HANDLERS[type(layer)])

    res = handler(container, layer)
    if isinstance(res, Err):
        LOG.error(
            "Could not spawn layer %s in container %s",
            str(layer),
...
def handle_layer_error(
    err: Err[ProcessExecutionError], container: model.Container, layer: model.Layer
) -> None:
    """
    ...
    image.events.append(events.LayerCreationFailed(str(layer), image.name, str(err)))
...
def store_failed_build(
    tag: str, container_id: str
) -> Result[str, ProcessExecutionError]:
    """
    ...
    if isinstance(res, Err):
        LOG.error(
            "Could not store failed build %s in tag %s",
            str(container_id),
...
class BuildahImageRegistry(ImageRegistry):
    ...
    def _remove(self, image: model.Image) -> None:
        ...
        if isinstance(res, Err):
            LOG.error(
                "Could not delete image %s. Reason: %s",
                image.name.lower(),
                str(res.unwrap_err()),
            )
