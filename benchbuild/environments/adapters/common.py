...
def container_cmd(base: BaseCommand) -> BaseCommand:
    ...
    def wrapped_cmd(*args: str) -> BaseCommand:
        ...
        if path_longer_than_50_chars(str(root)):
            LOG.error("%s - %s", root.__to_env_var__(), __MSG_SHORTER_PATH_REQUIRED)

        if path_longer_than_50_chars(str(runroot)):
            LOG.error("%s - %s", runroot.__to_env_var__(), __MSG_SHORTER_PATH_REQUIRED)
...
def run(cmd: BaseCommand, **kwargs: tp.Any) -> Result[str, ProcessExecutionError]:
    try:
...
def run_tee(
    cmd: BaseCommand, **kwargs: tp.Any
) -> Result[tp.Any, ProcessExecutionError]:
    try:
...
def run_fg(cmd: BaseCommand, **kwargs: tp.Any) -> Result[tp.Any, ProcessExecutionError]:
    try:
