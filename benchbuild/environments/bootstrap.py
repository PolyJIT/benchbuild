...
def bus() -> Messagebus:
    ...
    evt_handlers = {
        events.LayerCreated: [handlers.bootstrap(ui.print_layer_created, images_uow)],
        events.ImageCreated: [handlers.bootstrap(ui.print_image_created, images_uow)],
    ...
    cmd_handlers = {
        commands.CreateImage: handlers.bootstrap(handlers.create_image, images_uow),
        commands.CreateBenchbuildBase: handlers.bootstrap(
            handlers.create_benchbuild_base, images_uow
        ),
        commands.RunProjectContainer: handlers.bootstrap(
            handlers.run_project_container, containers_uow
        ),
        commands.ExportImage: handlers.bootstrap(
            handlers.export_image_handler, images_uow
        ),
        commands.ImportImage: handlers.bootstrap(
            handlers.import_image_handler, images_uow
        ),
        commands.DeleteImage: handlers.bootstrap(
            handlers.delete_image_handler, images_uow
        ),
    }
