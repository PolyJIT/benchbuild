...
class BenchBuildContainerRun(cli.Application):
    ...
    image_export = cli.Flag(
        ["export"], default=False, help="Export container images to EXPORT_DIR"
    )

    image_import = cli.Flag(
        ["import"], default=False, help="Import container images from EXPORT_DIR"
    )
    ...
    mount_tmp_dir = cli.Flag(
        ["mount-tmp-dir"], default=False, help="Mount benchbuild's tmp directory."
    )
    ...
    def main(self, *projects: str) -> int:
        ...
        tasks = {
            ...
            "Run": partial(
                run_experiment_images,
                wanted_experiments,
                wanted_projects,
                not self.no_mount_build_dir,
                self.mount_tmp_dir,
            ),
        }
...
class BenchBuildContainerRemoveImages(cli.Application):
    ...
    delete_project_images = cli.Flag(
        ["with-projects"], default=False, help="Delete project images too"
    )
    ...
    def main(self, *projects: str) -> int:
        ...
        tasks = {
            "Remove selected images": partial(
                remove_images,
                wanted_experiments,
                wanted_projects,
...
def cli_process(
    ...
    if unknown_experiments:
        print(
            "Could not find ", str(unknown_experiments), " in the experiment registry."
        )
...
def export_image(image_name: str) -> None:
    ...
    export_path = local.path(CFG["container"]["export"].value) / export_name + ".tar"
...
def import_image(image_name: str) -> None:
    ...
    import_path = local.path(CFG["container"]["import"].value) / import_name + ".tar"
...
def create_project_images(experiments: ExperimentIndex, projects: ProjectIndex) -> None:
    """
...
def create_experiment_images(
    ...
    for exp in enumerate_experiments(experiments, projects):
        for prj in exp.projects:
            ...
            image_tag = make_image_name(f"{exp.name}/{prj.name}/{prj.group}", version)
...
def remove_images(
    experiments: ExperimentIndex, projects: ProjectIndex, delete_project_images: bool
) -> None:
    """
