...
def partial_order_to_grid(poset, n):
    ...
    sort, i must be < j for every (i,j) in the poset.
    """
...
def vr_topsort(n, m):
    ...
    generate all linear extensions of a poset based on an initial valid
    linear extension as described in

     Yaakov L. Varol and Doron Rotem, An Algorithm to Generate All
    ...
    {(1,3),(2,3),(3,4)} (which can be passed to partial_order_to_grid to
    generate the incidence matrix 'm')
...
if __name__ == '__main__':
    n = 6
    poset = [(1, 2), (1, 3), (2, 4), (3, 5), (4, 6), (5, 6)]
    grid = partial_order_to_grid(poset, n)
    result = vr_topsort(n, grid)
