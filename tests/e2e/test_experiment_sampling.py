...
from benchbuild.utils import log, run, slurm
...
class SampleExperiment(bb.Experiment):
    ...
    @classmethod
    def sample(cls, prj_cls: bb.project.ProjectT) -> tp.Sequence[source.Revision]:
...
def test_sampling_is_used_by_benchbuild_run(mocked_sample, project_cls):
    ...
    """
    ngn = engine.Experimentator(experiments=[SampleExperiment], projects=[project_cls])
...
@mock.patch("tests.e2e.test_experiment_sampling.SampleExperiment.sample")
def test_sampling_is_used_by_benchbuild_slurm(mocked_sample, tmp_path, project_cls):
    """
    Check, if a generated plan has used the experiment's sample method.
